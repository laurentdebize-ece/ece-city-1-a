...
void dessinerMap(Vector2 mapPosition,int i,int j){
    ...
    DrawRectangleLines(mapPosition.x,mapPosition.y, LARGEUR1CASE, LARGEUR1CASE, GRAY);
...
void placementBatiment(Map map[45][35],Vector2 mapPosition,int i,int j){
    if (map[i][j].etat == 0) {  //Quoi faire en fonction de l'etat de la case (voir map.h)
        ...
        if (map[i][j].stade == 0) {
            DrawRectangle(mapPosition.x,mapPosition.y, LARGEUR1CASE * 3 , LARGEUR1CASE * 3  , RED);
        }
        if (map[i][j].stade == 1) {
            DrawRectangle(mapPosition.x,mapPosition.y, LARGEUR1CASE * 3  , LARGEUR1CASE * 3  ,
                          GREEN);
        }
        if (map[i][j].stade == 2) {
            DrawRectangle(mapPosition.x,mapPosition.y, LARGEUR1CASE * 3  , LARGEUR1CASE * 3  ,
        ...
        DrawRectangle(mapPosition.x,mapPosition.y, LARGEUR1CASE * 4, LARGEUR1CASE * 6, YELLOW);
    } else if (map[i][j].etat == 3) {
        DrawRectangle(mapPosition.x,mapPosition.y, LARGEUR1CASE * 4, LARGEUR1CASE * 6, BLUE);
    }
}
    ...
    if (collisionMAP != 0) {
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
            for (int k = 0; k < 6; k++) {
                ...
                if (hud[3].etat == 1 && map[i][j].occupe == 0 && map[i - 1][j + 1].occupe == 0 &&
                    map[i - 1][j - 1].occupe == 0 && map[i + 1][j + 1].occupe == 0 && map[i + 1][j - 1].occupe == 0) {
                    for (int a = -2; a < 3; a++) {
                        for (int b = -4; b < 5; b++) {
                            map[i][j].etat = 3;
...
void boucle(Map map[45][35],HUD hud[6],Vector2 mapPosition,Vector2 mapSize,Vector2 mousePosition,int evolution,int time,int collisionHUD,int collisionMAP) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < 45; i++) {
            for (int j = 0; j < 35; j++) {
                mapPosition.x = i * LARGEUR1CASE + LARGUEUR/5; //Coordonnée de la map
                mapPosition.y = j * LARGEUR1CASE + HAUTEUR/6;
                MAP.x = i * LARGEUR1CASE + LARGUEUR/5; //Coordonnée de la map
                MAP.y = j * LARGEUR1CASE + HAUTEUR/6;
